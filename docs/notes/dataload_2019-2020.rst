...
* Establishment Means lookup
  ...
  * Prep: Get lookup table from Annie Simpson
    * New: Lookup by ITIS TSN first, if that fails, lookup by scientific name

* Geo lookup

  * Prep: 
    * Get US counties shapefile from Shayne and Canada shapefile from Denver
    * Merge US and Canada shapefiles into a single shapefile with centroids.
      ...
      * Create a LUT for faster lookup.
    ...
    * Simplify the marine EEZ shapefile into a shapefile with smaller, simpler
      ...
      * Replace MULTIPOLYGON features with the component simple POLYGON features.  
        ...
        polygons.
      * Create a 2.5 or 5 degree global grid (experimented with 0.01, 0.10, 1, 
        2.5, 5, 10 degree resolution).
