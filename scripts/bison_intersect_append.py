...
def get_s3_dynframe(s3_bucket, s3_path):
    ...
    print(f"Read {full_path} with {full_path.count()} records into dynamic frame.")
    bison_points_dynf.printSchema()
    return bison_points_dynf


# ----------------------------------------------------
def read_rds(secret, db_name, schema, tbl_name):
    # Load polygon table from Amazon RDS PostgreSQL
...
# Get spatial table
polygon_table = read_rds(secret, rds_instance, rds_db_schema, county_table)
...
intersected_points = intersected_points.withColumn("georef_st", col("polygon_table.STUSPS"))
job.commit()