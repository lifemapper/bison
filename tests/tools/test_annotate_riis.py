...
class Test_annotate_riis:
    ...
    def test_read_small_riis(self):
        ...
        nnsl = NNSL(fn_args["_test_small_riis_filename"], logger, is_annotated=False)
        # Read original species data
        nnsl.read_riis()
        assert len(nnsl.by_taxon) == fn_args["_test_small_riis_name_count"]
        assert len(nnsl.by_riis_id) == fn_args["_test_small_riis_record_count"]
        assert (
    ...
    def test_read_original_riis(self):
        ...
        fn_args = get_test_parameters(script_name)
        logger = Logger(script_name, None)
        nnsl = NNSL(fn_args["riis_filename"], logger, is_annotated=False)
        # Read original species data
        nnsl.read_riis()
        assert len(nnsl.by_taxon) == fn_args["_test_riis_name_count"]
        assert len(nnsl.by_riis_id) == fn_args["_test_riis_record_count"]
        assert (
    ...
    def test_read_annotated_riis(self):
        ...
        fn_args = get_test_parameters(script_name)
        logger = Logger(script_name, None)
        nnsl = NNSL(fn_args["_test_annotated_riis_filename"], logger, is_annotated=True)
        # Read original species data
        nnsl.read_riis()
        assert len(nnsl.by_taxon) == fn_args["_test_annotated_riis_name_count"]
        assert len(nnsl.by_riis_id) == fn_args["_test_annotated_riis_record_count"]
        assert (
